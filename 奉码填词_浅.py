...
def Generate(source,template):
    result = shallow_generate_lyrics(template,source)
    return result
...
class MyMainWindow(QWidget):
    ...
    def ininUI(self):
        ...
        palette = QPalette()
        palette.setBrush(self.backgroundRole(), QBrush(QPixmap("./gui/6.jpg")))
        ...
        self.text1.setStyleSheet("background-image:url(./gui/6.jpg)")

        self.text2 = QTextEdit('请在这里输入你想要生成歌词的格式',self)
        self.text2.selectAll()
        self.text2.setFocus()
        self.text2.setGeometry(220,60,150,30)
        self.text2.setStyleSheet("background-image:url(./gui/6.jpg)")
